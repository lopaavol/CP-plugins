...
# Modified from CellProfiler 3.x version to CellProfiler 4.x 2021
...
from cellprofiler_core.setting.subscriber import FileImageSubscriber
from cellprofiler_core.constants.measurement import (IMAGE, EXPERIMENT)
from cellprofiler_core.constants.module import (USING_METADATA_TAGS_REF, USING_METADATA_HELP_REF, IO_FOLDER_CHOICE_HELP_TEXT, IO_WITH_METADATA_HELP_TEXT)
from cellprofiler_core.preferences import get_absolute_path, get_headless
from cellprofiler_core.preferences import (DEFAULT_INPUT_FOLDER_NAME, DEFAULT_OUTPUT_FOLDER_NAME, ABSOLUTE_FOLDER_NAME, \
...
"""%globals())
    ...
    def validate_module(self, pipeline):
        ...
        if self.wants_file_name_suffix.value:
            ...
            if len(undefined_tags) > 0:
                raise ValidationError("%s is not a defined metadata tag. Check the metadata specifications in your load modules" %
                                     undefined_tags[0],
    ...
    def validate_module_warnings(self, pipeline):
        ...
        if pipeline.test_mode:
            raise ValidationError("ExportToACC will not produce output in Test Mode",
                                      self.directory)
    ...
    def upgrade_settings(self, setting_values, variable_revision_number, module_name):
        ...
        if variable_revision_number == 1:
            setting_values = (setting_values[:4] + [NO, ""] + setting_values[4:])
            variable_revision_number = 2
