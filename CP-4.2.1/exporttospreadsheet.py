...
per-object measurements."""
        ...
        self.wants_agg_mean_well = Binary(
            "Calculate the per-well mean values for object measurements?",
            False,
            doc="""\
Select *"Yes"* for **ExportToSpreadsheet** to calculate population
...
per-object measurements."""
        ...
        self.wants_agg_median_well = Binary(
            "Calculate the per-well median values for object measurements?",
            False,
            doc="""\
Select *"Yes"* for **ExportToSpreadsheet** to calculate population
statistics over all the objects in each well and save that value as an
...
per-object measurements."""
        ...
        self.wants_agg_std_well = Binary(
            "Calculate the per-well standard deviation values for object measurements?",
            False,
            doc="""\
Select *"Yes"* for **ExportToSpreadsheet** to calculate population
statistics over all the objects in each well and save that value as an
...
""".format(
    ...
    def run(self, workspace):
        ...
        if self.show_window:
            ...
            if self.wants_agg_mean_well or self.wants_agg_median_well or self.wants_agg_std_well:
                columns.append((FTR_WELL, self.make_well_file_name(
                    FTR_WELL, workspace, image_set_number)))
    ...
    def make_well_file(self, image_set_numbers, workspace, settings_group=None):
        ...
        file_name = self.make_well_file_name(
            FTR_WELL, workspace, image_set_numbers[0], settings_group)
        ...
        for object_feature in object_features:
            if object_feature[1] == 'Count':
                ...
                if self.wants_agg_mean_well:
                    well_features.append(get_agg_measurement_name(AGG_MEAN, object_feature[0], object_feature[1]))
                if self.wants_agg_median_well:
                    well_features.append(get_agg_measurement_name(AGG_MEDIAN, object_feature[0], object_feature[1]))
                if self.wants_agg_std_well:
                    well_features.append(get_agg_measurement_name(AGG_STD_DEV, object_feature[0], object_feature[1]))

        well_features.insert(0, FTR_WELL)
        well_images = {}
        well_positions = []
            ...
            for well_pos in well_positions:
                ...
                for object_name, feature_name in object_features:
                    ...
                    if feature_name == 'Count':
                        ...
                        feature_measurements = numpy.concatenate(feature_measurements)
                    if feature_name == 'Count':
                        count = sum(feature_measurements)
                        values.append(count)
                    else:
                        if self.wants_agg_mean_well:
                            meanvalue = numpy.mean(feature_measurements)
                            values.append(meanvalue)
                        if self.wants_agg_median_well:
                            medianvalue = numpy.median(feature_measurements)
                            values.append(medianvalue)
                        if self.wants_agg_std_well:
                            stdvalue = numpy.std(feature_measurements, ddof=1)
                            values.append(stdvalue)
                    for value in values:
                        if value is None:
                            row.append('')
                        elif isinstance(value, bytes):
                            row.append(value.decode('unicode_escape', errors='ignore'))
