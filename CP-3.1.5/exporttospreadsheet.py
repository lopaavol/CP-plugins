...
per-object measurements.""" % globals())
        ...
        self.wants_agg_mean_well = cps.Binary(
                "Calculate the per-well mean values for object measurements?", False, doc="""\
Select *"%(YES)s"* for **ExportToSpreadsheet** to calculate population
statistics over all the objects in each well and save that value as an
aggregate measurement in the Well file. For instance, if you are
measuring the area of the Nuclei objects and you check the box for this
option, **ExportToSpreadsheet** will create a column in the Well file
called “Mean\_Nuclei\_AreaShape\_Area”. Note that this setting can
generate a very large number of columns of data.
...
per-object measurements.""" % globals())
        ...
        self.wants_agg_median_well = cps.Binary("Calculate the per-well median values for object measurements?",
                                                  False, doc="""\
Select *"%(YES)s"* for **ExportToSpreadsheet** to calculate population
statistics over all the objects in each well and save that value as an
aggregate measurement in the Well file. For instance, if you are
...
per-object measurements.""" % globals())
        ...
        self.wants_agg_std_well = cps.Binary(
                "Calculate the per-well standard deviation values for object measurements?", False, doc="""\
Select *"%(YES)s"* for **ExportToSpreadsheet** to calculate population
statistics over all the objects in each well and save that value as an
aggregate measurement in the Well file. For instance, if you are
measuring the area of the Nuclei objects and you check the box for this
...
produced in the pipeline, as well as per-image, per-well, per-experiment and
...
""" % globals()))
    ...
    def settings(self):
        ...
        result = [self.delimiter, self.add_metadata,
                  ...
                  self.wants_overwrite_without_warning, self.wants_agg_mean_well,
                  self.wants_agg_median_well, self.wants_agg_std_well]
        ...
        if self.show_window:
            ...
            if self.wants_everything:
                ...
                for i in range(len(self.object_groups)):
                    ...
                    if last_in_file:
                        first = True
            if self.wants_agg_mean_well or self.wants_agg_median_well or self.wants_agg_std_well:
    ...
    def make_well_file_name(self, object_name, workspace, image_set_number, settings_group=None):
        '''
        object_name - name of the objects whose measurements are to be
                            saved (or IMAGES or EXPERIMENT)
        workspace - workspace containing metadata measurements
        image_number - the first image number in the group being written
        settings_group - the settings group asking for the file to be written
    ...
    def make_well_file(self, image_set_numbers, workspace, settings_group=None):
        ...
        object_names = self.filter_object_names(object_names)

        object_features = []
        object_counts = []
        for object_name in object_names:
            ofeatures = self.filter_columns(m.get_feature_names(object_name), object_name)
            object_features += [(object_name, feature) for feature in ofeatures]
