...
class ExportToSpreadsheet(cpm.CPModule):
    ...
    def create_settings(self):
        ...
        self.wants_agg_mean_well = cps.Binary(
            ...
            Select <i>%(YES)s</i> for <b>ExportToSpreadsheet</b> to calculate statistics over all the objects in each well 
            and store the results as "per-well" columns. For instance, 
            if you are measuring the area of the Nuclei objects and you check the aggregate
            mean box in this module, <b>ExportToSpreadsheet</b> will create a table in the database called
            ...
            <p><i>Note:</i> this option is only available if you have extracted well metadata 
            from the filename using the <b>Metadata</b> or <b>LoadData</b> modules. %(USING_METADATA_HELP_REF)s'''%globals())
        ...
        self.wants_agg_median_well = cps.Binary(
            ...
            available if you have extracted well metadata from the filename using 
            the <b>Metadata</b> or <b>LoadData</b> modules. %(USING_METADATA_HELP_REF)s'''%globals())
        ...
        self.wants_agg_std_dev_well = cps.Binary(
            ...
            <p><i>Note:</i> this option is only
            available if you have extracted well metadata from the filename 
            using the <b>Metadata</b> or <b>LoadData</b> modules. %(USING_METADATA_HELP_REF)s'''%globals())
    ...
    def settings(self):
        ...
        result = [self.delimiter, self.add_metadata, 
                  ...
                  self.wants_aggregate_std, self.wants_agg_mean_well,
                  self.wants_agg_median_well, self.wants_agg_std_dev_well, self.directory,
    ...
    def visible_settings(self):
        ...
        result += [ self.wants_aggregate_means, self.wants_aggregate_medians,
                    self.wants_aggregate_std, self.wants_agg_mean_well, self.wants_agg_median_well,
                    self.wants_agg_std_dev_well, self.wants_genepattern_file]
        ...
        if self.show_window:
            ...
            if self.wants_everything:
                ...
                for i in range(len(self.object_groups)):
                    ...
                    if last_in_file:
                        first = True
            if self.wants_agg_mean_well or self.wants_agg_median_well or self.wants_agg_std_dev_well:
                columns.append((cpmeas.FTR_WELL, self.make_well_file_name(
                    cpmeas.FTR_WELL, workspace, image_set_number)))
    ...
    def run_objects(self, object_names, workspace, settings_group = None):
        ...
        for metadata_group in metadata_groups:
            if len(object_names) == 1 and object_names[0] == IMAGE:
                ...
                if self.wants_agg_mean_well or self.wants_agg_median_well or self.wants_agg_std_dev_well:
                    self.make_well_file(metadata_group.image_numbers,
                                        workspace, settings_group)
    ...
    def make_well_file_name(self, object_name, workspace, image_set_number, settings_group=None):
        '''
        :param object_name: name of the objects whose measurements are to be
                            saved (or IMAGES or EXPERIMENT)
        workspace - workspace containing metadata measurements
        ...
        '''
        file_name = self.make_objects_file_name(
            object_name, workspace, image_set_number, settings_group)
        #file_name = os.path.join(os.path.dirname(file_name),"Well.csv")
        return file_name

    def make_well_file(self, image_set_numbers, workspace, settings_group=None):
        """Make a file containing well measurements
        ...
        """
        m = workspace.measurements
        file_name = self.make_well_file_name(
            cpmeas.FTR_WELL, workspace, image_set_numbers[0], settings_group)
        object_names = m.get_object_names()
        object_features = []
        object_counts = []
        for object_name in object_names:
            if object_name in [EXPERIMENT, IMAGE]:
                continue
            object_features += [(object_name, feature) for feature in m.get_feature_names(object_name)]
            object_counts += [(object_name, 'Count')]
        object_features = object_counts + object_features

        well_features = []
        for object_feature in object_features:
            if object_feature[1] == 'Count':
                well_features.append(object_feature[1] + '_' + object_feature[0])
            else:
                if self.wants_agg_mean_well:
                    well_features.append(cpmeas.get_agg_measurement_name(cpmeas.AGG_MEAN, object_feature[0], object_feature[1]))
                if self.wants_agg_median_well:
                    well_features.append(cpmeas.get_agg_measurement_name(cpmeas.AGG_MEDIAN, object_feature[0], object_feature[1]))
                if self.wants_agg_std_dev_well:
                    well_features.append(cpmeas.get_agg_measurement_name(cpmeas.AGG_STD_DEV, object_feature[0], object_feature[1]))

        well_features.insert(0, cpmeas.FTR_WELL)
        well_images = {}
        well_positions = []
        for image_num in image_set_numbers:
            well_position = m.get_measurement(IMAGE,'Metadata_Well',image_num)
            well_image_num = well_images.get(well_position,[])
            if not well_image_num:
                well_positions.append(well_position)
            well_image_num.append(image_num)
            well_images[well_position] = well_image_num

        if not self.check_excel_limits(workspace, file_name,
                                       len(well_images.keys()),
                                       len(well_features)):
            return
        ...
        try:
            ...
            writer.writerow(well_features)
            for well_pos in well_positions:
                row = [well_pos]
                image_nums = well_images[well_pos]
                for object_name, feature_name in object_features:
                    if feature_name == 'Count':
                        feature_measurements = m.get_measurement(object_name, 'Number_Object_Number', image_nums)
                    else:
                        feature_measurements = m.get_measurement(object_name, feature_name, image_nums)
                    values = []
                    if feature_name == 'Count':
                        feature_measurements = [x[-1] if x.size > 0 else 0 for x in feature_measurements]
                    elif feature_name == 'Number_Object_Number':
                        feature_measurements = [x[-1] if x.size > 0 else 0 for x in feature_measurements]
                    else:
                        feature_measurements = np.concatenate(feature_measurements)

                    if feature_name == 'Count':
                        count = sum(feature_measurements)
                        values.append(count)
                    else:
                        if self.wants_agg_mean_well:
                            meanvalue = np.mean(feature_measurements)
                            values.append(meanvalue)
                        if self.wants_agg_median_well:
                            medianvalue = np.median(feature_measurements)
                            values.append(medianvalue)
                        if self.wants_agg_std_dev_well:
                            stdvalue = np.std(feature_measurements)
...
Do you want to save it anyway?""" %
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if variable_revision_number == 1 and not from_matlab:
            ...
            setting_values = (setting_values[:6] + [cps.NO, cps.NO, cps.NO, cps.NO, cps.NO, cps.NO] + 
                              setting_values[6:])
            variable_revision_number = 2
        ...
        if variable_revision_number == 11 and not from_matlab:
            #setting_values = setting_values[:7] + [cps.NO, cps.NO, cps.NO] + setting_values[7:]
            variable_revision_number = 12
